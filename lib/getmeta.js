...
var presentation = {
    ...
    'S_ARDUINO_NODE':          {index: 17, role: '',                  vars: ['I_BATTERY_LEVEL', 'I_BATTERY_VOLTAGE', 'I_SKETCH_VERSION', 'I_VERSION', 'I_SKETCH_NAME', 'I_CONFIG']},
    //'S_ARDUINO_RELAY':         {index: 18, role: '',                  vars: []},
    'S_ARDUINO_REPEATER_NODE': {index: 18, role: '',                  vars: [/*'I_BATTERY_LEVEL',*/ 'I_SKETCH_VERSION', 'I_SKETCH_NAME'/*, 'I_CONFIG'*/]},
    ...
    //'S_RGBW_STATE':	           {index: 200,role: 'rgbw.light',        vars: ['V_STATUS', 'V_VAR1', 'V_VAR2', 'V_VAR3', 'V_VAR4', 'V_RGBW', 'V_VAR5']}
    'S_RGBW_STATE':	           {index: 200,role: 'rgbw.light',        vars: ['on', 'r', 'g', 'b', 'ww', 'cw', 'rgbw', 'transition', 'program']}
    
};

var vars = {
    'r':                    {index: 201, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'g':                    {index: 202, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'b':                    {index: 203, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'ww':                   {index: 204, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'cw':                   {index: 205, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'rgbw':                 {index: 206, role: 'value',     type: 'string',     read: true, write: true},     //Custom value
    'transition':           {index: 207, role: 'value',     type: 'number',     read: true, write: true},     //Custom value
    'on':                   {index: 208, role: 'value',     type: 'boolean',    read: true, write: true},     //Custom value
    'program':              {index: 209, role: 'value',     type: 'string',     read: true, write: true},     //Custom value
    ...
    'V_IMPEDANCE':          {index: 14, role: 'value',     type: 'number',     read: true, write: false, def: 0, min: 0, unit: 'Ohm'},    //Impedance value
    ...
    'V_LEVEL':              {index: 37, role: 'level',     type: 'number',     read: true, write: false, def: 0},                                  //Used for sending level-value
    ...
    'I_BATTERY_VOLTAGE':    {index: 250,role: 'value',     type: 'number',     read: true, write: false, def: 0, unit: 'V'},
    'I_DEBUG':              {index: 28, role: 'state',     type: 'string',     read: true, write: false, def: ''}, // debug
...
function _getVarEntry(type) {
    var ret = vars[type];
    return ret == undefined ? vars['V_STATUS'] : ret;
}
...
//function _getId(ip, idt) {
//    if (id['id'] != undefined) id = id.id;
//    return (ip ? ip.replace(/\./g, '_') + '.'  : '') + id;
//}

function _getName(ip, packet) {
    return _getId(ip, packet) + '.' + _fullChildId(packet.childId);
}

function _useChildId (packet) {
    //return true ? ':' + _fullChildId(packet.childId) : '';
    return false ? ':' + _fullChildId(packet.childId) : '';
}

function _getChildName(packet, subType, id) {
    var ret = packet.payload ? packet.payload + _useChildId(packet) + (subType ? '.' + subType : '') : id;
    return ret;
}

function deviceIdFromPacket(ip, packet) {
    //var id = _getName(ip, packet) + (packet.subType ? '.' + packet.subType : '');
    var id = _getName(ip, packet) + ((packet.subType && packet.num.type != 0) ? '.' + packet.subType : '');
    return id;
}
    ...
    var result = [
        {
            ...
            common: {
                ...
                name:       packet.payload ? packet.payload + _useChildId(packet) : _getName(ip, packet),
                role:       type.role
            },
            native: {
                ip:         ip,
        ...
        if (variable) {
            result.push({
                ...
                common: {
                    ...
                    name:       _getChildName(packet, type.vars[v], id),
                    type:       variable.type,
                    lastChange: "",
...
module.exports.meta = {
    deviceIdFromPacket: deviceIdFromPacket,
    fullChildId: _fullChildId,
    _getId: _getId,
    _getVarEntry: _getVarEntry,
