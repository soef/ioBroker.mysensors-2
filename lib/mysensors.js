...
function MySensorsUDP(config, log, onCreated) {
    ...
    if (config.mode === 'server') {
        ...
        //this._interface.send(buf, 0, buf.length, config.port || 5003, '255.255.255.255', function (err) {
        
        //todo: alle vorandenen devices durchgehen...
        //this._interface.send(buf, 0, buf.length, config.port || 5003, '192.168.1.53', function (err) {
        //});
    } else {
    }
    this.zeroInterface = function () {
    ...
    // this.restartConnectionTimer = function (client, timeout) {
    //     if (client.oInterval) return; //clearInterval(client.oInterval);
    //     if (adapter.config.connTimeout) client.oInterval = setInterval(function() {
    //         this.write('0;0;3;0;18;heartbeat', client.ip);
    //     }.bind(this), adapter.config.connTimeout);
    // };
...
function MySensors(_adapter, onCreated) {
    adapter = _adapter;
